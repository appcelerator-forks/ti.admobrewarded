...
#define TAG "net.birdirbir.admobrewarded"
...
static void TiModule_getBinding(const FunctionCallbackInfo<Value>& args)
{
	...
	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "net.birdirbir.admobrewarded TiModule_getBinding() requires 1 argument: binding");
	...
	if (maybeBinding.IsEmpty()) {
		titanium::JSException::Error(isolate, "net.birdirbir.admobrewarded TiModule_getBinding requires 1 argument: binding. Received argument that could not be converted to a String");
		args.GetReturnValue().Set(scope.Escape(Undefined(isolate)));
		return;
	}
...
Java_net_birdirbir_admobrewarded_TiModuleBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("net.birdirbir.admobrewarded", &TiModuleBinding);
